...
Option --database=<database name> is optional. The database needs to be selected
if there are multiple database in the XML file.
Option --mech=<filename> is optional. When a mechanism file is provided, only the
processes of listed species are parsed.
Option --insert is optional. It enables automatically inserting the parsed process
into the mechanism file.
Option --phase=<phase name> is optional. It indicates which phase in the mechanism
file.
Option --output=<filename> is optional. If the output file name is not given, an
output file will be the same name as the input file, with the extension changed
to '.yaml'.
"""
...
def convert(
        ...
        mechfile: str = Optional[str],
        phase: str = Optional[str],
        insert: bool = True,
        outfile: Union[str, Path] = Optional[Union[str, Path]],
    ) -> None:
    ...
    if mechfile is not None:
        if not useCantera:
            ...
            gas = ct.Solution(mechfile, transport_model=None)
...
def registerProcess(process: etree.Element,
                    process_list: List[etree.Element],
                    gas: ct.Solution=None):
    """Register a collision process (electron collision reaction) to form a
    list for output.
