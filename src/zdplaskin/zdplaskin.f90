...
! config
!
  integer, parameter :: species_max = 8, species_electrons = 8, species_length = 4, reactions_max = 1, reactions_length = 16
...
!
  ...
  integer, parameter, private               :: bolsig_species_max = 6, bolsig_species_length = 3 
...
subroutine ZDPlasKin_fex(neq,t,y,ydot)
  ...
  rrt(1) = rrt(1) * density(1)**2 * density(8) 
  ydot(1) = -rrt(1) 
  ydot(2) = 0.0d0
  ydot(3) = 0.0d0
  ydot(4) = 0.0d0
...
subroutine ZDPlasKin_jex(neq,t,y,ml,mu,pd,nrpd)
  ...
  density(:) = y(1:species_max)
  call ZDPlasKin_reac_rates(t)
  pd(1,1) = pd(1,1) - rrt(1) * density(1) * density(8) * 2.0d0
  pd(1,8) = pd(1,8) - rrt(1) * density(1)**2 
  pd(7,1) = pd(7,1) + rrt(1) * density(1) * density(8) * 2.0d0
  pd(7,8) = pd(7,8) + rrt(1) * density(1)**2 
  pd(8,1) = pd(8,1) - rrt(1) * density(1) * density(8) * 2.0d0
  pd(8,8) = pd(8,8) - rrt(1) * density(1)**2 
