...
void WeakIonGasElectron::calculateChemionScatRate()
{
    double T = m_thermo->temperature();
    // Chemionization rate coefficient unit: [1/s]
    double k = 1.09e-11 * pow(T/300.0, -2.19) * std::exp(-165.10/T) * 1e-6;
    double X_CH = m_thermo->moleFraction("CH");
    double X_O = m_thermo->moleFraction("O");
    double X_E = m_thermo->moleFraction("E");
    if (X_E != 0.0) {
        m_chemionScatRate = k * X_CH * X_O / X_E;
    }
}
...
void WeakIonGasElectron::calculateDistributionFunction()
{
    ...
    if (m_setChemionScatRate == false) {
        calculateChemionScatRate();
    }
