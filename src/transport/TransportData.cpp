...
GasTransportData::GasTransportData(
        ...
        double dispersion_, double qua_polar_)
    : geometry(geometry_)
    , diameter(diameter_)
    ...
    , dispersion_coefficient(dispersion_)
    , quadrupole_polarizability(qua_polar_)
{
}
        ...
        double dispersion_, double qua_polar_)
{
    ...
    dispersion_coefficient = 1e-50 * dispersion_; // convert from Angstroms^5 to m^5
    quadrupole_polarizability = 1e-50 * qua_polar_; // convert from Angstroms^5 to m^5
}
...
void setupGasTransportData(GasTransportData& tr, const XML_Node& tr_node)
{
    ...
    double qua = 0.0;
    getOptionalFloat(tr_node, "quadrupole_polarizability", qua);

    tr.setCustomaryUnits(geometry, diam, welldepth, dipole, polar,
                         rot, acentric, dispersion, qua);
}
